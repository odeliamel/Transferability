...
parser.add_argument('--epochs', type=int, default=2000, help='number of epochs to train (defaulta: 10)')
...
parser.add_argument('--logdir', default='log/fc1_new/shafi_advtrain_testand1train_projected', help='folder to save to the log')
parser.add_argument('--data_root', default='/tmp/public_dataset/pytorch/', help='folder to save the model')
parser.add_argument('--decreasing_lr', default='40,80', help='decreasing strategy')
parser.add_argument('--weight_factor', type=int, default=100000, help='modelclass')
...
def train(model=None, train_loader=None, test_loader=None, args=None):
    if args is None:
    ...
    model.load_state_dict(
        torch.load("/home/odeliam/PycharmProjects/Transferability/MNIST/log/fc1_new/mnist_clean1200/init.pth"))
    ...
    V = load_PCA("pca100q10iters.pickle")["V"]
    ...
    attack = PGDAttack(predict=clean_model, eps=1., nb_iter=1, eps_iter=1.,
    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                run_adv_epoch(attack, global_projection_matrix, args, batch_idx, data, epoch, F.cross_entropy,
...
def run_adv_epoch(attack, global_projection_matrix, args, batch_idx, data, epoch, loss_fn, model, optimizer, target, train_len, print):
    # print("step")
    optimizer.zero_grad()
    model.eval()
    adv_perturbation = attack.perturb(data, target) - data

    batch_size = adv_perturbation.shape[0]
    ...
    flat_batch_v = adv_perturbation.reshape(batch_size, 784, 1)
    ...
    res = res.reshape_as(adv_perturbation)
...
if __name__ == '__main__':
    args = parser.parse_args()
    global_dims = [100]
    log = 'log/fc1_new/projected/shafi_advtrain_testand1train_projected_pca100q_sameinit_'
    for d in global_dims:
        args.global_manifold_dim = d
        args.logdir = log + str(d)
