...
def perturb_iterative(xvar, yvar, predict, project, nb_iter, eps, eps_iter, loss_fn,
    ...
    for ii in range(nb_iter):
        ...
        if ord == np.inf:
            ...
            # delta.data = project(xvar.data + delta.data) - xvar.data
            # print("2", torch.norm(delta.data, p=2))
            delta.data = clamp(xvar.data + delta.data, clip_min, clip_max
                               ) - xvar.data
            if eps is not None:
                delta.data = clamp_by_pnorm(delta.data, ord, eps)
            # print("1", torch.norm(delta.data))
            delta.data = project(delta.data, xvar.data)
            # print("2", torch.norm(delta.data))
